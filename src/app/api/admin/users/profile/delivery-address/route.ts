...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({success:true, usersaddress, status: 200 });
  } catch (error) {
...
export async function POST(request:Request) {
  try {
    ...
    return NextResponse.json({success:true, message: "User Address Added Successfully.", status: 200 });
  } catch (error) {
    console.error("❌ Error fetching users:", error);
    return NextResponse.json({success:false, message: "Failed to save shipping address" }, { status: 500 });
  }
}
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({success:true, message: "User Address Updated Successfully.", status: 200 });
  } catch (error) {
    console.error("❌ Error fetching users:", error);
...
export async function DELETE(request: Request) {
  try {
    ...
    if(data && data.is_default ===true){
      return NextResponse.json({success:false, error: "Default Address cannot be removed" , status: 404 });
    }
    const respo = await UserAddress.deleteOne({_id: adid});
    return NextResponse.json({success:true, message: "User Address Removed Successfully.", status: 200 });
  } catch (error) {
    console.error("❌ Error deleting address:", error);
    return NextResponse.json({success:false, error: "Failed to remove address", status: 500 });
  }
}